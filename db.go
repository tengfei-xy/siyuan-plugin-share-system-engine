...
func db_check_table() error {
	...
	log.Infof("数据库无数据！")
	// if _, err := app.db.Exec(`CREATE TABLE share (	id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,	appid VARCHAR(40),	docid VARCHAR(30),	title TEXT,	link VARCHAR(30),	access_key VARCHAR(10) DEFAULT '',	access_key_enable INTEGER DEFAULT 0,	update_time DATETIME DEFAULT CURRENT_TIMESTAMP,	expire_time DATETIME,	count INTEGER DEFAULT 0,	status INTEGER DEFAULT 0,	UNIQUE(appid, docid)  )`); err != nil {
...
func dbTableGET() (string, error) {
	...
	// var update_time_str, expire_time_str string
